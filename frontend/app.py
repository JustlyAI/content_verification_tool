...
def export_document(document_id: str, chunking_mode: str, output_format: str) -> Optional[dict]:
    """
    ...
    """
    try:
        cprint(f"[FRONTEND] Exporting document: {document_id} ({chunking_mode} -> {output_format})", "cyan")
        payload = {
...
def main():
    ...
    with st.spinner("Checking backend connection..."):
        if not check_backend_health():
            st.error("⚠️ Backend API is not available. Please ensure the backend server is running.")
    ...
    with st.sidebar:
        ...
        st.markdown("""
        ### Features
        - **Document Upload**: PDF or DOCX files (max 100 MB)
        - **Chunking Modes**:
          - Paragraph-level
          - Sentence-level
        - **Output Formats**:
    ...
    st.markdown("Upload a PDF or DOCX file (maximum 100 MB)")
